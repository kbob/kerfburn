...
# XXX This script knows too much about the build system.
# Could it be something like
#
#   gen-pin-mappings --mcu MCU-FILE [--template=TEMPLATE-FILE] file...
#
# It could write to stdout and the Makefile could direct it into the
# generated file.
#
# The "-M" flag might prevent that.
...
autogen_disclaimer = '''
     ...
     *
     *    script:         %(script)s.
     *    pin mappings:   %(mapping)s
     *    MCU definition: %(mcu)s
     *    template:        %(template)s
     */
'''
...
def make_pin_definitions(mcu_port_pins, mcu_timer_pins, mcu_interrupt_pins):
    ...
    def def_timer(timer, desc, emit_ident=True):
        ...
        add_blank_line()
        add_def(ident + '_TCCRC', 'TCCR%(timer)sC' % locals())
        add_blank_line()
    ...
    def def_timer_pin(pin, desc, **kwargs):
        ...
        add_def(ident + '_FOC', 'FOC%(timer)s%(comp)s' % locals())
...
def emit_autogen_disclaimer(outf):

    ag = autogen_disclaimer % files
    lines = ag.expandtabs().splitlines()
        ...
        for line in inf:
            if '%' in line:
                if '%PIN_DEFINITIONS%' in line:
                    ...
                    continue
                if '%AUTOGEN_DISCLAIMER%' in line:
                    emit_autogen_disclaimer(outf)
                    continue
...
def print_dependencies(mcu, output):
    print '%s: %s' % (output, ' '.join(files.values()))
